...
test.describe("POST - post", async () => {
    ...
    const $tagsSubItem = "//ul[@class = 'wp-submenu wp-submenu-wrap']/li/a[text() = 'Tags']";
    const $resultMsg = "//div[contains(@class, 'notice') and not(contains(@class, 'hidden'))]/p";
    ...
    test("@POST_TAG_001 - Tag - add tag failed", async ({ page }) => {
        ...
        await test.step("Add tag with existed name", async () => {
            ...
            await expect(page.locator($resultMsg)).not.toHaveText(expectErrorBlankTagName);
            await expect(page.locator($resultMsg)).toHaveText(expectErrorDuplicateExistTagName);
    ...
    test("@POST_TAG_002 - Tag - add tag success", async ({ page }) => {
        ...
        await test.step("Delete all new added data", async () => {
            ...
            await page.locator(`//a[@aria-label="Delete “${validTagName1}”"]`).click();

            await expect(page.locator($tagName1Td)).not.toBeVisible();

            await page.locator($tagName2Td).hover();
            await page.locator(`//a[@aria-label="Delete “${validTagName2}”"]`).click();
    ...
    test("@POST_TAG_003 - Tag - slug auto remove special character", async ({ page }) => {
        ...
        await test.step("Delete all new added data", async () => {
            ...
            await page.locator(`//a[@aria-label="Delete “${specialTagName}”"]`).click();
    ...
    test("@POST_CATEGORY_001 - Category - create category success", async ({ page }) => {
        ...
        await test.step("Delete all new added Categories", async () => {
            ...
            await page.locator(`//a[@aria-label="Delete “${categoryName2}”"]`).click();
